...
def serie_single_processor(serie):
    ...
    id_arr = id_arr/np.average(id_arr)
    t_arr = np.array(t_datalist[0]['data'][0])

    plt.plot(t_arr,id_arr)
    plt.ticklabel_format(axis='both',style='sci', scilimits=(0,0))
    plt.title("Id normalized versus time of: " + serie['title']) 
    plt.ylabel('Id/Id,avg(A)')
    plt.xlabel('T(s)')

    outputdir = PureWindowsPath(sys.argv[2])
    filename = Path(outputdir/PureWindowsPath("id_versus_time_normalized\\" + serie['title'] + ".png"))    
...
outputdir = PureWindowsPath(outputdir + '\\id_versus_time_normalized')
if not Path(outputdir).is_dir():
    Path(outputdir).mkdir()

filename = Path(scriptdir/PureWindowsPath('data\\id_versus_time_normalized.json'))
if not Path(outputdir).is_dir():
    Path(outputdir).mkdir()
